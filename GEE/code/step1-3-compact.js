    ...
    // Wilayah Studi: Provinsi Jambi, Indonesia (2016-2025)
    // Sensor Optik: Landsat 8/9 OLI & Sentinel-2 MSI (Harmonized)
    // Sensor Api: MOD14A1 (MODIS)
    // =====================================================================
    ...
    var CONFIG = {
        ...
        assetPath: 'users/faarisnew/fire-drone-2026-v3/', // UBAH INI
        exportScale: 30,
        hotspotExportScale: 375,
        exportCRS: 'EPSG:32648',
        MAX_PIXELS: 1e13,
        driveFolder: 'GEE_FireDrone_Jambi_v5'
    };
    ...
    // TAHAP 1: KOMPOSIT TAHUNAN (HYBRID SENSOR STRATEGY)
    var yearlyComposites = ee.ImageCollection(years.map(function (y) {
        ...
        // [SOLUSI] Gunakan Landsat murni untuk tahun < 2022
        // Sentinel-2 baru digabungkan mulai tahun 2022 ke atas saat kualitas SCL optimal
        var combined = ee.Algorithms.If(
            y.lt(2022),
            l8Col.merge(l9Col),
            l8Col.merge(l9Col).merge(s2Col)
        );

        return ee.ImageCollection(combined).median().clip(jambiROI).set('year', y);
    }));
    ...
    var yearlyHotspots = ee.ImageCollection(years.map(function (y) {
        ...
        return modisActive.rename('hotspot_count').clip(jambiROI).set('year', y);
    }));
    ...
    Export.table.toDrive({ collection: deforValidationStats, description: 'Task_Stats_Deforestasi_Drive', folder: CONFIG.driveFolder, fileFormat: 'CSV' });
    Export.table.toDrive({ collection: burnValidationStats, description: 'Task_Stats_Burn_Drive', folder: CONFIG.driveFolder, fileFormat: 'CSV' });

    var hotspotCounts = ee.FeatureCollection(years.map(function (y) {
        var hsImg = ee.Image(yearlyHotspots.filter(ee.Filter.eq('year', y)).first());
        var hsSum = hsImg.gt(0).reduceRegion({
            reducer: ee.Reducer.sum(), geometry: jambiROI,
            scale: 1000, maxPixels: CONFIG.MAX_PIXELS
        });
        return ee.Feature(null, hsSum).set('year', y);
    }));

    Export.table.toDrive({
        collection: hotspotCounts,
        description: 'Task_Stats_Hotspot_Count',
        folder: CONFIG.driveFolder, fileFormat: 'CSV'
    });
